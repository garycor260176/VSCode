...
const char* devicename = "counter_02"; // sets MQTT topics and hostname for ArduinoOTA

const char* ssid = "default";     //wi-fi netwrok name
const char* password = "nthfgtdn";  //wi-fi network password
const char* mqtt_server = "192.168.1.40";   // mqtt broker ip address (without port)
...
char mqtt_serial_ip_ch_cache[50];
char mqtt_serial_load_ch_cache[50];

int mqtt_counter = sprintf(mqtt_serial_publish_ch_cache,"%s%s%s","counters/", devicename,"/counter");
const PROGMEM char* mqtt_serial_publish_ch = mqtt_serial_publish_ch_cache;
int mqtt_distance = sprintf(mqtt_serial_publish_distance_ch_cache,"%s%s%s","counters/", devicename,"/distance");
const PROGMEM char* mqtt_serial_publish_distance_ch = mqtt_serial_publish_distance_ch_cache;
int mqtt_receiver = sprintf(mqtt_serial_receiver_ch_cache,"%s%s%s","counters/", devicename,"/receiver");
const PROGMEM char* mqtt_serial_receiver_ch = mqtt_serial_receiver_ch_cache;
int mqtt_ip = sprintf(mqtt_serial_ip_ch_cache,"%s%s%s","counters/", devicename,"/ip");
const PROGMEM char* mqtt_serial_ip_ch = mqtt_serial_ip_ch_cache;
int mqtt_load = sprintf(mqtt_serial_load_ch_cache,"%s%s%s","counters/", devicename,"/load");
const PROGMEM char* mqtt_serial_load_ch = mqtt_serial_load_ch_cache;
...
void callback(char* topic, byte *payload, unsigned int length) {
    ...
    if (newTopic == mqtt_serial_receiver_ch) 
    {
      ...
      else if (newPayload == "zones_calibration")
      {
        zones_calibration();
      }
...
void setup(void)
{
  ...
  */
  ...
  if(!started) {
    started = true;
    String stringaCounter = WiFi.localIP().toString();
    stringaCounter.toCharArray(peopleCounterArray, stringaCounter.length() +1);
    client.publish(mqtt_serial_ip_ch, peopleCounterArray);

    stringaCounter = "1";
    stringaCounter.toCharArray(peopleCounterArray, stringaCounter.length() +1);
    client.publish(mqtt_serial_load_ch, peopleCounterArray);
  }
}
