...
void Msg_distance( const String &message ){
  if(ROI_state.distance == message.toInt( )) return;
  ROI_state.distance = message.toInt();
  write_eeprom();
}
...
void onConnection(){
  ...
  client.Subscribe("settings/distance", Msg_distance); 
}
...
//Serial.println("Z " + String(Zone) + " dist = " + String(distance));
//Serial.println(String(ROI_state.ROI_size_h) + " " + String(ROI_state.ROI_size_w) + " " + ROI_state.center[Zone]);
  switch(Zone){
    case 0:
      ...
      if(state.zone1.state != cur_state.zone1.state)
        Serial.println("Zone " + String(Zone) + ", state = " + String(state.zone1.state));
    ...
    case 1:
      ...
      if(state.zone2.state != cur_state.zone2.state)
        Serial.println("Zone " + String(Zone) + ", state = " + String(state.zone2.state));
    break;
  }
