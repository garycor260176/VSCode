...
int State_1 = LOW;
/*
void IRAM_ATTR interrupt_1() { 
  if((millis() - ContactTime_1) > 15 ) { // debounce of sensor signal 
    newData_1 = true;
    ContactTime_1 = millis(); 
    State_1 = digitalRead(INT_1);
  } 
} 
*/
...
int State_2 = LOW;
/*void IRAM_ATTR interrupt_2() { 
  if((millis() - ContactTime_2) > 15 ) { // debounce of sensor signal 
    newData_2 = true;
...
void setup() {
  ...
  pinMode(INT_2, INPUT);
  delay(100);
  //attachInterrupt(INT_1, interrupt_1, CHANGE); 
  //attachInterrupt(INT_2, interrupt_2, CHANGE); 
...
//    newData_2 = false;  
  State_2 = digitalRead(INT_2);
  ContactTime_2 = ContactTime_1; //время
